...
class WechatOpen
{
    ...
    const EVENT_WXAPP_AUDIT_SUCCESS = 'weapp_audit_success';   //小程序审核通过通知
    const EVENT_WXAPP_AUDIT_FAIL = 'weapp_audit_fail';        //小程序审核失败通知
    const EVENT_USER_ENTER_TEMPSESSION = 'user_enter_tempsession';        //用户进入小程序客户消息
    ...
    const MP_BASE_URL_PREFIX = 'https://mp.weixin.qq.com'; //几个授权接口需要使用此前缀
    ...
    /**
     * 设置sdk的authorizer_refresh_token,sdk自动初始化后一般appid还未知，需要开发人员在业务逻辑代码中设置到sdk
     */
    ...
    /**
     * 获取sdk的authorizer_refresh_token
     */
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 */
    protected function http_post($url,$param,$post_file=false,$need_apiclient=false,$pem_cert='',$pem_key=''){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
	 ...
	 */
	public function getJsSign($url, $timestamp=0, $noncestr='', $appid='',$authorizer_refresh_token=''){
	    if (!$this->jsapi_ticket && !$this->getJsTicket($appid,$authorizer_refresh_token) || !$url) return false;
	    if (!$timestamp)
	        $timestamp = time();
	    if (!$noncestr)
	 ...
	 */
	public function getJsCardTicket($api_ticket='',$appid='', $authorizer_refresh_token='' ){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
		if (!$appid) $appid = $this->appid;
	 ...
	 */
	public function getServerIp($appid='',$authorizer_refresh_token ='') {
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 */
	public function createMenu(array $data, $appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 */
	public function getMenu( $appid='',  $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 * @return boolean
	 */
	public function deleteMenu($appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 *
	 */
	public function addconditionalMenu($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 * @return bool
	 */
	public function delconditionalMenu($data, $appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 */
	public function trymatchMenu($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 * @param type 类型：图片:image 语音:voice 视频:video 缩略图:thumb  注意：小程序帐号仅支持 图片:image
     * @param $appid 公众号appid
     * @param $authorizer_refresh_token 公众号authorizer_refresh_token
     * @return boolean|array
	 */
	public function uploadMedia($data, $type,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return raw data
	 */
	public function getMedia($media_id,$is_video=false,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		//原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
	 ...
	 * @return boolean|array
	 */
	public function uploadImg($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		//原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
     ...
     * @return boolean|array
     */
    public function uploadForeverMedia($data, $type,$is_video=false,$video_info=array(), $appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean|array
     */
    public function uploadForeverArticles($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean|array
     */
    public function updateForeverArticles($media_id,$data,$index=0,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     */
    public function getForeverMedia($media_id,$is_video=false,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean
     */
    public function delForeverMedia($media_id,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array('media_id' => $media_id);
     ...
     * )
     */
    public function getForeverList($type,$offset,$count,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array(
            'type' => $type,
            'offset' => $offset,
     ...
     * )
     */
    public function getForeverCount($appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function uploadArticles($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 *  }
	 */
	public function uploadMpVideo($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function sendMassMessage($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function sendGroupMassMessage($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function deleteMassMessage($msg_id,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function previewMassMessage($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * }
	 */
	public function queryMassMessage($msg_id,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 */
	public function getQRCode($scene_id,$type=0,$expire=604800,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 */
	public function getShortUrl($long_url,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	    $data = array(
            'action'=>'long2short',
            'long_url'=>$long_url
	 ...
	 */
	public function getDatacube($type,$subtype,$begin_date,$end_date='',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 */
	public function getUserList($next_openid='',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
	 ...
	 */
	public function getUserInfo($openid, $lang = 'zh_CN',$appid='', $authorizer_refresh_token=''){
		if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
	 */
	public function getUsersInfo($openids,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function updateUserRemark($openid,$remark,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	    $data = array(
			'openid'=>$openid,
			'remark'=>$remark
	 ...
	 * @return bool|mixed
	 */
	public function createTags($name,$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'tag'=>array(
	 ...
	 * @return bool|mixed
	 */
	public function updateTags($id,$name,$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'tag'=>array(
	 ...
	 * @return bool
	 */
	public function deleteTags($id,$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'tag'=>array(
	 ...
	 * @return bool|mixed
	 */
	public function getTagsUser($tagid,$next_openid = '',$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'tagid'=>$tagid,
	 ...
	 * @return bool
	 */
	public function batchtaggingTagsMembers($tagid,$openid_list,$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'tagid'=>$tagid,
	 ...
	 * @return bool
	 */
	public function batchuntaggingTagsMembers($tagid,$openid_list =array(),$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'tagid'=>$tagid,
	 ...
	 * @return bool|mixed
	 */
	public function getidlistTags($openid,$appid='', $authorizer_refresh_token='')
	{
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
			'openid'=>$openid,
	 ...
	 * @return boolean|array
	 */
	public function getGroup($appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 */
	public function getUserGroup($openid,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	    $data = array(
	            'openid'=>$openid
	    );
	 ...
	 * @return boolean|array
	 */
	public function createGroup($name,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
	 ...
	 * @return boolean|array
	 */
	public function updateGroup($groupid,$name,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
	 ...
	 * @return boolean|array
	 */
	public function updateGroupMembers($groupid,$openid,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
				'openid'=>$openid,
				'to_groupid'=>$groupid
	 ...
	 * @return boolean|array
	 */
	public function batchUpdateGroupMembers($groupid,$openid_list,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		$data = array(
				'openid_list'=>$openid_list,
				'to_groupid'=>$groupid
     ...
     * @return boolean|array
     */
    public function sendTestCustomMessage($data ,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
...
//		$result = $this->http_get(self::API_BASE_URL_PREFIX.self::OAUTH_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
	...
	public function getOauthUserinfo($access_token,$openid){
		$result = $this->http_get(self::API_BASE_URL_PREFIX.self::OAUTH_USERINFO_URL.'access_token='.$this->access_token.'&openid='.$openid);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 * @return boolean|array
	 */
	public function setTMIndustry($id1,$id2='',$appid='', $authorizer_refresh_token=''){
	 ...
	 */
	public function addTemplateMessage($tpl_id,$appid='', $authorizer_refresh_token=''){
	 ...
	 * @return boolean|array
	 */
	public function sendTemplateMessage($data, $appid, $authorizer_refresh_token){
		if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
     ...
     * 下发小程序和公众号统一的服务消息
     * @return bool|mixed
     * author Bear <a5566123@vip.qq.com>
	 ...
	 * @return boolean|array
	 */
	public function getCustomServiceMessage($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function getCustomServiceKFlist($appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 */
	public function getCustomServiceOnlineKFlist($appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * }
	 */
	public function createKFSession($openid,$kf_account,$text='',$appid='', $authorizer_refresh_token=''){
	    $data=array(
	    	"openid" =>$openid,
	 ...
	 * }
	 */
	public function closeKFSession($openid,$kf_account,$text='',$appid='', $authorizer_refresh_token=''){
	    $data=array(
	    	"openid" =>$openid,
	        "kf_account" => $kf_account
	    );
	 ...
	 *  }
	 */
	public function getKFSession($openid,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 *  )
	 */
	public function getKFSessionlist($kf_account,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 *  )
	 */
	public function getKFSessionWait($appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * }
	 */
	public function addKFAccount($account,$nickname,$password,$appid='', $authorizer_refresh_token=''){
	    $data=array(
	    	"kf_account" =>$account,
	        "nickname" => $nickname,
	 ...
	 * }
	 */
	public function updateKFAccount($account,$nickname,$password,$appid='', $authorizer_refresh_token=''){
	    $data=array(
	            "kf_account" =>$account,
	            "nickname" => $nickname,
	            "password" => md5($password)
	 ...
	 * }
	 */
	public function deleteKFAccount($account,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * }
	 */
	public function setKFHeadImg($account,$imgfile,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array
	 */
	public function querySemantic($uid,$query,$category,$latitude=0,$longitude=0,$city="",$region="",$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	    $data=array(
	            'query' => $query,
	            'category' => $category,
     ...
     */
    public function createCard($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean
     */
    public function updateCard($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean
     */
    public function delCard($card_id,$appid='', $authorizer_refresh_token='') {
        $data = array(
            'card_id' => $card_id,
        );
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     */
    public function getCardInfo($card_id,$appid='', $authorizer_refresh_token='') {
        $data = array(
            'card_id' => $card_id,
        );
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * }
     */
    public function getUserCardList($openid,$card_id,$appid='', $authorizer_refresh_token='') {
        $data = array(
            'openid' => $openid,
            'card_id' => $card_id
        );
     ...
     */
    public function getCardColors($appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return boolean|array   返回数组请参看 微信卡券接口文档 的json格式
     */
    public function getCardLocations($offset=0,$count=0,$appid='', $authorizer_refresh_token='') {
	    $data=array(
	    	'offset'=>$offset,
	        'count'=>$count
	    );
     ...
     */
    public function addCardLocations($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean|string
     */
    public function createCardQrcode($card_id,$code='',$openid='',$expire_seconds=0,$is_unique_code=false,$balance='',$appid='', $authorizer_refresh_token='') {
        $card = array(
            'card_id' => $card_id
        );
        $data = array(
     ...
     * }
     */
    public function consumeCardCode($code,$card_id='',$appid='', $authorizer_refresh_token='') {
     ...
     *  }
     */
    public function decryptCardCode($encrypt_code,$appid='', $authorizer_refresh_token='') {
        $data = array(
     ...
     * }
     */
    public function checkCardCode($code,$appid='', $authorizer_refresh_token='') {
        $data = array(
            'code' => $code,
        );
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * }
     */
    public function getCardIdList($offset=0,$count=50,$appid='', $authorizer_refresh_token='') {
        if ($count>50)
            $count = 50;
        $data = array(
            'offset' => $offset,
     ...
     * @return boolean
     */
    public function updateCardCode($code,$card_id,$new_code,$appid='', $authorizer_refresh_token='') {
        $data = array(
            'code' => $code,
            'card_id' => $card_id,
            'new_code' => $new_code,
     ...
     * @return boolean
     */
    public function unavailableCardCode($code,$card_id='',$appid='', $authorizer_refresh_token='') {
        $data = array(
            'code' => $code,
        );
        if ($card_id)
     ...
     * @return boolean
     */
    public function modifyCardStock($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean
     */
    public function updateMeetingCard($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean
     */
    public function activateMemberCard($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean|array
     */
    public function updateMemberCard($data,$appid='', $authorizer_refresh_token='') {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @return boolean|array
     */
    public function updateLuckyMoney($code,$balance,$card_id='',$appid='', $authorizer_refresh_token='') {
        $data = array(
                'code' => $code,
                'balance' => $balance
        );
     ...
     * @return boolean
     */
    public function setCardTestWhiteList($openid=array(),$user=array(),$appid='', $authorizer_refresh_token='') {
        $data = array();
        if (count($openid) > 0)
            $data['openid'] = $openid;
        if (count($user) > 0)
     ...
     * @copyright Show More
     */
    public function applyShakeAroundDevice($data,$appid='', $authorizer_refresh_token='' ){
        if (!$this->access_token && !$this->checkAuth($appid, $authorizer_refresh_token)) return false;
     ...
     */
    public function updateShakeAroundDevice($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @copyright Show More
     */
    public function searchShakeAroundDevice($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @copyright Show More
     */
    public function bindLocationShakeAroundDevice($device_id,$poi_id,$uuid='',$major=0,$minor=0,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if(!$device_id){
            if(!$uuid || !$major || !$minor){
                return false;
     ...
     * @copyright Show More
     */
    public function bindPageShakeAroundDevice($device_id,$page_ids=array(),$bind=1,$append=1,$uuid='',$major=0,$minor=0,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if(!$device_id){
            if(!$uuid || !$major || !$minor){
                return false;
     ...
     */
    public function uploadShakeAroundMedia($data,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @copyright Show More
     */
    public function addShakeAroundPage($title,$description,$icon_url,$page_url,$comment='',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array(
            "title" => $title,
            "description" => $description,
     ...
     * @copyright Show More
     */
    public function updateShakeAroundPage($page_id,$title,$description,$icon_url,$page_url,$comment='',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array(
            "page_id" => $page_id,
            "title" => $title,
     ...
     * @copyright Show More
     */
    public function searchShakeAroundPage($page_ids=array(),$begin=0,$count=1,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if(!empty($page_ids)){
            $data = array(
                'page_ids' => $page_ids
     ...
     * @copyright Show More
     */
    public function deleteShakeAroundPage($page_ids=array(),$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array(
            'page_ids' => $page_ids
        );
     ...
     * @copyright Show More
     */
    public function getShakeInfoShakeAroundUser($ticket,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * @copyright Show More
     */
    public function deviceShakeAroundStatistics($device_id,$begin_date,$end_date,$uuid='',$major=0,$minor=0,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if(!$device_id){
            if(!$uuid || !$major || !$minor){
                return false;
     ...
     */
    public function pageShakeAroundStatistics($page_id,$begin_date,$end_date,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array(
            'page_id' => $page_id,
            'begin_date' => $begin_date,
	 ...
	 */
	public function getOrderByID($order_id,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 */
	public function getOrderByFilter($status = null, $begintime = null, $endtime = null,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
	 ...
	 * @return bool
	 */
	public function setOrderDelivery($order_id, $need_delivery = 0, $delivery_company = null, $delivery_track_no = null, $is_others = 0,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		if (!$order_id) return false;
	 ...
	 * @return bool
	 */
	public function closeOrder($order_id,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
		if (!$order_id) return false;
     ...
     * }
     */
    public function getWxappSession($js_code,$appid='',$authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if (!$appid ) {
            $appid = $this->appid;
        }
     ...
     * }
     */
    public function wxaModifyDomain($action,$requestdomain='',$wsrequestdomain='',$uploaddomain='',$downloaddomain='',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
    ...
    /**
     * 微信小程序设置小程序业务域名（仅供第三方代小程序调用）
     *
     * @param string $access_token      //请使用第三方平台获取到的该小程序授权的authorizer_access_token
     * @param string $action            //add添加, delete删除, set覆盖, get获取。当参数是get时不需要填域名字段。如果没有action字段参数，则默认将开放平台第三方登记的小程序业务域名全部添加到授权的小程序中
     * @param string $webviewdomain     //合法域名，当action参数是get时不需要此字段。
     * @return boolean|array
     * 成功返回结果
     * {
     *   "errcode": 0,
     ...
     * }
     */
    public function wxaSetWebviewDomain($action,$webviewdomain='',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        //  if (!$action) return false;   //如果没有action字段参数，则默认将开放平台第三方登记的小程序业务域名全部添加到授权的小程序中
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_SETWEBVIEW_DOMAIN.'access_token='.$this->access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaBindTester($wechatid,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * }
     */
    public function wxaUnbindTester($wechatid,$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if (!$wechatid) return false;
     ...
     * @param string $appid      //请使用第三方平台获取到的该小程序授权的appid
     * @param string $authorizer_refresh_token      //请使用第三方平台获取到的该小程序授权的authorizer_refresh_token
     * @param array $item_list       //提交审核项的一个列表（至少填写1项，至多填写5项），具体参考文档https://open.weixin.qq.com/cgi-bin/showdocument?action=dir_list&t=resource/res_list&verify=1&id=open1489140610_Uavc4&token=&lang=zh_CN
     * @return boolean|array
     * 成功返回结果
     * {
     *   "errcode": 0,
     ...
     * }
     */
    public function wxaSubmitAudit($item_list,$appid='', $authorizer_refresh_token='' ){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = $item_list;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_SUBMIT_AUDIT.'access_token='.$this->access_token, self::json_encode($data));
     ...
     * }
     */
    public function wxaGetAuditstatus($auditid,$appid='', $authorizer_refresh_token='' ){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
    ...
    public function wxaRelease($appid='', $authorizer_refresh_token=''){
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_RELEASE.'access_token='.$this->access_token,'{}');
     ...
     *
     * @param string $appid      //请使用第三方平台获取到的该小程序授权的appid
     * @param string $authorizer_refresh_token      //请使用第三方平台获取到的该小程序授权的authorizer_access_token
     ...
     * @param Bool $auto_color      //自动配置线条颜色，如果颜色依然是黑色，则说明不建议配置主色调 默认false
     * @param Object $line_color      //auto_color 为 false 时生效，使用 rgb 设置颜色 例如 {"r":"xxx","g":"xxx","b":"xxx"} 十进制表示  默认 {"r":"0","g":"0","b":"0"}
     * @param Bool $is_hyaline     //是否需要透明底色， is_hyaline 为true时，生成透明底色的小程序码 默认 false
     * @return boolean|array
     * 成功返回结果
     * {
     *   "errcode": 0,
     ...
     * }
     */
    public function wxaCodeUnlimit($scene ,$page,$width = 430,$auto_color =false,$is_hyaline=false,$line_color = '{"r":"0","g":"0","b":"0"}',$appid='', $authorizer_refresh_token=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_CODE_UNLIMIT.'access_token='.$this->access_token,self::json_encode($data));
    ...
    public function getComponentAccessToken($component_verify_ticket){
        ...
        $rs = $this->getCache($componentAT);
      // if ($rs) {              //以数据库保存的有效token和过期时间为准
      ...
      //  }
        ...
        if ($result)
        {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in'])-700 : 7200;
            $this->setCache($componentAT,$json,$expire);
     ...
     * @param string      $redirect_url  授权后重定向的回调地址，请使用UrlEncode对链接进行处理。 注：要求redirect_url的域名要跟登记的业务域名一致，且业务域名不能带路径。 业务域名需登录公众号，在设置-公众号设置-功能设置里面对业务域名设置。
     * @param string|null $reserved  用于保持请求和回调的状态，授权请后原样带回给第三方。该参数可用于防止csrf攻击（跨站请求伪造攻击），建议第三方带上该参数，可设置为简单的随机数加session进行校验，开发者可以填写a-zA-Z0-9的参数值，最多128字节，要求做urlencode
     *
     * @return string
     */
    public function getSubscribemsgUrl( $scene,  $template_id, $redirect_url, $reserved, $appid='', $authorizer_refresh_token='')
    {
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if(!$appid) $appid = $this->appid;
        ...
        return self::MP_BASE_URL_PREFIX.self::WX_SUBSCRIBE_MESSAGE.http_build_query($queries)."#wechat_redirect";
    }

    /**
     * 第二步：通过API推送订阅模板消息给到授权微信用户
     * @param string $appid      //所需跳转到的小程序appid（该小程序appid必须与发模板消息的公众号是绑定关联关系，并且小程序要求是已发布的）
     * @param string $access_token      //请使用第三方平台获取到的该小程序授权的authorizer_access_token
     * @param string $openid           //填接收消息的用户openid
     * @param string $template_id           //订阅消息模板ID
     * @param string|null $url           //点击消息跳转的链接，需要有ICP备案
     * @param string|null $miniprogram           //跳小程序所需数据，不需跳小程序可不用传该数据
     * @param string|null $pagepath           //所需跳转到小程序的具体页面路径，支持带参数,（示例index?foo=bar）
     * @param string $scene           //订阅场景值
     * @param string $title           //消息标题，15字以内
     * @param data $data           //消息正文，value为消息内容文本（200字以内），没有固定格式，可用\n换行，color为整段消息内容的字体颜色（目前仅支持整段消息为一种颜色）
     * @return boolean|array
     * 成功返回结果
     * {
     *   "errcode": 0,
     ...
     * }
     */
    public function templateSubscribe($openid, $template_id, $url , $pagepath,$scene,$title,$data,$appid='', $authorizer_refresh_token='' ){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
 ...
 * 解密
 * */
    public function decrypt($timeStamp,$nonce,$msg_sign,$text,$valid = true){

        $encodingAesKey = $this->encodingAesKey;
        $token = $this->component_access_token;
